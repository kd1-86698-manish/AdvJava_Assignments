...
public class TeamsDaoImpl implements TeamsDao {
	...
	public String deleteTeamDetails(Long teamId) {
		...
		String mesg = "Deletion team failed!!!!!!";
		Teams team = null;
		Session session = getSessionFactory().getCurrentSession();
		Transaction tx = session.beginTransaction();
		try {
	...
	@Override
	public Teams getTeamAndPlayers(Long teamId) {
		...
		try {
			...
			team = session.createQuery(jpql, Teams.class).setParameter("id", teamId).getSingleResult();
